...
export const createUserProfile = async (userId, profileData = {}) => {
    try {
        ...
        if (existingProfile) {
            console.warn(`Profile already exists for user ${userId}. Returning existing profile.`);
...
// @desc    Get user profile by Profile ID (for admin or specific use cases)
...
export const getUserProfileByUserId = async (req, res) => {
    try {
        ...
        if (!profile) {
            return res.status(404).json({ success: false, message: 'User profile not found for this user ID' });
        }

        res.status(200).json({ success: true, profile });

    } catch (error) {
        console.error(`Error fetching user profile by User ID ${req.params.userId}:`, error);
         // Handle invalid ObjectId format for the userId parameter
